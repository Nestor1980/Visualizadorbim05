...
// Only sync work here — model is NOT yet registered in the worker when onItemSet fires.
...
// All worker-dependent operations go here. Call AFTER ifcLoader.load() resolves.
  ...
  model.object.traverse((child: THREE.Object3D) => {
    if (child instanceof THREE.Mesh) {
      ...
      if (Array.isArray(child.material)) {
        child.material = child.material.map((m: THREE.Material) => convertToPhong(m));
      } else {
